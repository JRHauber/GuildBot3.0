...
def rollDice(dMax=1, mod=0, dNum=1, name=""):
    ...
    if(dNum > 1):
        ...
        for x in range(0, dNum):
            ...
            result += int(mod)
            result2 = "(" + str(result-int(mod)) + ") " + str(result)
            sum += result
            ...
            if(x < dNum):
                dRes += result2 + ", "
        dRes = dRes[:-2]
        dRes += "]` Total: `" + str(sum) + "` Average: `" + str(math.floor(sum/dNum)) + "`"
    else:
        dRes += " `["
        result = int(np.floor(random.uniform(1, int(dMax)+1)))
        result += int(mod)
...
async def add(ctx, arg1, arg2):
    ...
    newChar = character(arg1, ctx.author.id, dice, adder,
                        health, health, generateCID())
    ...
    if(len(args) == 0):
        if(findSelected(ctx.author.id) == 0):
            ...
            if (1 + int(characterList[cPos]['health'])) > int(characterList[cPos]['maxhealth']):
                ...
                characterList[cPos]['health'] = int(
                    characterList[cPos]['health']) + 1
                writeTable(characterList, 'characters')
        ...
        if(findSelected(ctx.author.id) == 0):
            ...
            if (int(args[0]) + int(characterList[cPos]['health'])) > int(characterList[cPos]['maxhealth']):
                ...
                characterList[cPos]['health'] = int(
                    characterList[cPos]['health']) + int(args[0])
                writeTable(characterList, 'characters')
                await ctx.send("Set your health to: " + str(characterList[cPos]['health']))
    ...
    if(len(args) == 0):
        if(findSelected(ctx.author.id) == 0):
            ...
            if ((int(characterList[cPos]['health']) - 1) <= 0) or ((int(characterList[cPos]['health']) - 1) <= -1):
                characterList[cPos]['health'] = (
                    int(characterList[cPos]['health']) - 1)
                writeTable(characterList, 'characters')
    ...
    if (args[0] == 'add'):
        ...
        if (args[1] == '1'):
            ...
            for x in range(len(initiativeList1)):
                ...
                output += str(x+1) + " - " + characterList[cPos]['name'] + " - " + str(
                    characterList[cPos]['health']) + "/" + str(characterList[cPos]['maxhealth']) + "\n"
            output += "```"
        ...
        role = discord.utils.find(
            lambda r: r.name == "Event Runner", ctx.message.guild.roles)
    ...
    for server in bot.guilds:
        for channel in server.text_channels:
            if channel.name.lower() == "general":
                try:
                    await channel.send("<BOT> " + msg, delete_after=300)
                except Exception:
                    continue
                else:
                    break
